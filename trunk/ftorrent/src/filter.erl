...
%%%%
%% This module is intended to use for filtering out values of one table/list from other tables/lists. Also there is a function that joins two lists to one and sorts it later.
...
%% Filter out all occurrences/elements of list two from list one, and return list one
filter_values([], _Mine) ->
    [];
...
%% Add list two to list one and sort the list one, and return the joined list
sort_added(L, []) ->
    lists:sort(L);
sort_added(L, [H|T]) ->
    sort_added([H|L], T).
...
%% Filter out a given list('pieces we have') from all the set of peers/lists. This would return a list of 'wish pieces' from each peer or 'the pieces we need'
filter_peer_set([], _L) ->
    [];
...
%% Join all the peers' filtered-have-list into an histogram dictionary and return this dictionary
join_set(Peers,Me) ->
    Dic = orddict:new(),
...
%% Get the rarest set by sorting the given dictionary in ascending order. N = 2 in this case. 
get_rarest(D) ->
    sort_hist(D, 2).


%% Update the given peer's(pid) list by finding the peer and adding a new 'piece'. Return the list of all peers have-list. Takes a list of peers, piece number, pid, and an accumulator/'[]'    
...
%% Lookup a peer-have-list by Pid number in a list of peers. If finds one return it else return 'pid_not_there' msg
lookup([], _Pid) ->
    pid_not_there;
