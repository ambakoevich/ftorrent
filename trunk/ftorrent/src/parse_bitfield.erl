...
%% @version v0.1
%% @doc Created: 28-Nov-2011, Parse bitfield extracts indexes of pieces
%% another peer 'has'.

-module(parse_bitfield).
-export([decimal_to_bit/2, concat_list/1, define_index/2]).

%% @doc Convert a decimal into a list of bits.
...
%% @doc Concatenate lists of bits/numbers.
%% Arguments are a list of binary, and [] as an accumulator.
concat_list(Bin) ->
...
%% @doc Find a list of indices/pieces a peer has.
define_index([], _Index) ->
    [];
